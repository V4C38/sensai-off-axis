...
import { ALL_SPLAT_INDICES, SplatIndex } from '../utils/sceneConfig';

interface CalibrationWizardProps {
  onComplete: (calibration: CalibrationData) => void;
  onSkip: () => void;
  onClose: () => void;
  selectedSplat: SplatIndex;
  onSplatChange: (index: SplatIndex) => void;
  isSequencePlaying: boolean;
  onSequencePlayingChange: (isPlaying: boolean) => void;
}
...
const CalibrationWizard: React.FC<CalibrationWizardProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-black max-w-md w-full max-h-[calc(100vh-2rem)] overflow-y-auto p-8 relative border border-gray-800 rounded-sm">
        <button
          onClick={onClose}
        ...
        <div className="mt-6 pt-4 border-t border-gray-800">
          <div className="flex items-center justify-between gap-3 mb-3">
            <p className="text-xs text-gray-400">Splat</p>
            <button
              type="button"
              onClick={() => onSequencePlayingChange(!isSequencePlaying)}
            ...
            >
              {isSequencePlaying ? 'Play: On' : 'Play: Off'}
            </button>
          </div>

          <div className="grid grid-cols-5 gap-2">
            {ALL_SPLAT_INDICES.map((index) => {
              const isSelected = selectedSplat === index;
              return (
                <button
                  key={index}
                  onClick={() => onSplatChange(index as SplatIndex)}
